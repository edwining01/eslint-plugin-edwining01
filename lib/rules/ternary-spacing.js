...
const isTokenOnSameLine = require('../utils/isTokenOnSameLine')
...
module.exports = {
  ...
  create(context) {
    ...
    function hasLeadingSpace(token) {
      const tokenBefore = sourceCode.getTokenBefore(token)
      return tokenBefore && isTokenOnSameLine(tokenBefore, token) &&
    ...
    function checkOperatorSpacing(token, node) {
      ...
      if(tokenIsQuestionMark) {
        ...
        if(expectSpaceAfterQuestionMark) {
          if(!tokenHasTrailingSpace && !isLastTokenInCurrentLine(token))
